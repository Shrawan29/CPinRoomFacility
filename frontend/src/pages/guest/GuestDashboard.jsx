...
export default function GuestDashboard() {
  ...
  // ── Quick Actions — 4 unique items, no duplicates ─────────────────────────
  const quickActions = [
  ...
  return (
    <div className={`fixed inset-0 bg-[#F6EADB] flex flex-col overflow-hidden transition-opacity duration-500 ${fadeIn ? "opacity-100" : "opacity-0"}`}>
      ...
      >
        ...
        {/* ── QUICK ACTIONS — 2×2 grid, all 4 unique ──────────────────────── */}
          ...
          <div className="grid grid-cols-2 gap-3">
            {quickActions.map((action) => (
              ...
              >
                ...
                <p className="text-[15px] font-semibold text-[#1F1F1F] leading-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                  {action.label}
                </p>
          ...
          {hasActiveOrder ? (
            <div className="bg-white rounded-[18px] border border-[#c9a96e]/18 shadow-[0_2px_12px_rgba(30,21,16,0.06)] p-4">
              ...
              {activeOrder.items.map((item) => (
                <div key={item.name} className="flex justify-between text-[13px] text-[#5c4a3e] font-light py-1.5 border-b border-[#c9a96e]/10 last:border-0">
                  <span>{item.name}</span>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <div className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#A4005D]" />}
              </button>
            );
          })}
        </div>
