      ...
      <section className="mt-6 mb-2">
        ...
        {hasActiveOrder ? (
          <GlassCard className="flex flex-col gap-3 px-5 py-5 animate-fadein" style={{ borderRadius: 20, boxShadow: "0 4px 18px rgba(0,0,0,0.07)" }}>
            <div className="text-base font-semibold mb-1">Active Order</div>
            <div className="text-sm text-[--text-muted] mb-2">
              {activeOrder.items.map((item, i) => (
                <span key={i}>{item.qty}x {item.name}{i < activeOrder.items.length - 1 ? " • " : ""}</span>
              ))}
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-[--text-muted]">Status: <span className="font-semibold text-[--text-primary]">{activeOrder.status}</span></span>
              <button className="rounded-full px-4 py-1 bg-[--brand] text-white font-semibold shadow hover:scale-105 transition-all" style={{ fontSize: 15 }}>
                Track Order
              </button>
            </div>
          </GlassCard>
          ...
          <GlassCard className="flex flex-col items-center justify-center gap-2 px-5 py-6 animate-fadein" style={{ borderRadius: 20, boxShadow: "0 4px 18px rgba(0,0,0,0.07)" }}>
            ...
            <button
              className="rounded-full px-5 py-2 bg-[--brand] text-white font-semibold shadow hover:scale-105 transition-all mb-1"
              style={{ fontSize: 15 }}
              onClick={() => navigate("/guest/menu")}
            >
              Order Now
            ...
            <button
              className="text-xs text-[--text-muted] underline mt-1"
